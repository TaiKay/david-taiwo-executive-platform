...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) { return <html lang="en" className={`${inter.variable} ${space.variable}`}><body><StructuredData data={identity}/><AnalyticsIntegrations/><a href="#main" className="focus-ring sr-only fixed left-4 top-4 z-[100] bg-navy px-4 py-2 text-white focus:not-sr-only">Skip to content</a><Navigation /><main id="main">{children}</main><Footer /><Analytics /></body></html>; }
